...
// user's email via --user <email> (or the REMOTE_CLAUDE_USER env var); we look up
// their sub in the Cognito pool, then read the namespaced SSM params.
const { spawnSync } = require('child_process');

const REGION = process.env.AWS_REGION || 'us-east-1';
const STACK = process.env.STACK_NAME || 'remote-claude';

function aws(...parts) {
  const r = spawnSync('aws', [...parts, '--region', REGION, '--output', 'text'], { encoding: 'utf8' });
  if (r.status !== 0) throw new Error(r.stderr?.trim() || `aws ${parts[0]} failed`);
  return r.stdout.trim();
}

// Pull --user <email> from argv (or REMOTE_CLAUDE_USER). Returns the email or null.
function userArg(argv) {
  const i = argv.indexOf('--user');
  if (i !== -1 && argv[i + 1]) return argv[i + 1];
  return process.env.REMOTE_CLAUDE_USER || null;
}
...
function resolveMvm(email) {
  if (!email) {
    throw new Error(
      ...
      + '  Pass --user <email>, or set REMOTE_CLAUDE_USER=<email>.\n'
  ...
  let mvmId, endpoint;
  try {
    mvmId = aws('ssm', 'get-parameter', '--name', `/remote-claude/users/${sub}/mvm-identifier`, '--query', 'Parameter.Value');
    endpoint = aws('ssm', 'get-parameter', '--name', `/remote-claude/users/${sub}/mvm-endpoint`, '--query', 'Parameter.Value');
  } catch (e) {
...
module.exports = { aws, userArg, resolveMvm, REGION, STACK };
